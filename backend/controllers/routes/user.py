...
users_router = APIRouter(tags=[Tags.USER])


@users_router.get("/user", summary="Get the current user (and its roles).")
...
@users_router.put("/user", summary="Modify the the current user")
...
@users_router.get("/users", summary="Get all users.")
...
@users_router.get("/users/{uid}", summary="Get a certain user.")
...
@users_router.put("/users/{uid}", summary="Modify the roles of a certain user.")
